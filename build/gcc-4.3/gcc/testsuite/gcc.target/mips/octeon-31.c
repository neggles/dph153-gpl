/* Check the *insvdi_clear_upper32 pattern.  */
...
#endif
struct s1
f1 (struct s1 s, unsigned long x)
{ unsigned long mask = 0xffffffffUL; s.x2 = x & mask; return s; }
struct s2
f2 (struct s2 s, unsigned long x)
{ unsigned long mask = 0xffffffffUL; s.x2 = x & mask; return s; }
struct s3
f3 (struct s3 s, unsigned long x)
{ unsigned long mask = 0xffffffffUL; s.x1 = x & mask; return s; }
struct s4
f4 (struct s4 s, unsigned long x)
{ unsigned long mask = 0xffffffffUL; s.x2 = x & mask; return s; }
